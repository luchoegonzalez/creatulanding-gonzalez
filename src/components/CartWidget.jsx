...
function CartWidget() {
  ...
  const baseStyles = "text-black rounded-xl p-2 duration-150 relative";
  const hoverStyles = "hover:text-white hover:cursor-pointer hover:bg-pink-600";
  const activeStyles = "text-white bg-pink-600";

  return (
    <Link
      to={'/cart'}
      className={`${baseStyles} ${hoverStyles} ${isActive ? activeStyles : ''}`}
    >
